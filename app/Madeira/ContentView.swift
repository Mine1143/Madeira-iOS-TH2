...
struct ContentView: View {
    ...
    // Any-game launcher + controller bridge
    @ObservedObject private var exeLibrary = ExeLibrary.shared
    @ObservedObject private var controllerBridge = ControllerBridge.shared
    @State private var showingExePicker = false
    ...
    var body: some View {
        ...
        NavigationStack {
            ...
            .onAppear {
                ...
                // MFi/Bluetooth controllers (Xbox/DualSense/DS4) → Wine keys/mouse
    ...
    private var actionButtons: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 12) {
                ...
                // Any-game launcher: scan drive_c for .exe files the user
                // dropped in (Files app / SideStore), pick one, play. This is
                // the generic path — no per-title button needed.
                Button("Custom EXE... (\(exeLibrary.games.count))") {
                    exeLibrary.scan()
                    showingExePicker = true
                }
                .buttonStyle(.borderedProminent)
                .tint(.indigo)
        ...
        .sheet(isPresented: $showingExePicker) {
            ExePickerSheet()
        }
    }
...
// MARK: - Any-game launcher sheet (Custom EXE)

/// Scans Documents/wine/drive_c for .exe files and launches the picked
/// one through the standard MADEIRA_EXE env contract — every game the
/// user copies into the container (via Files app: On My iPhone → Madeira
/// → wine → drive_c) shows up here.
struct ExePickerSheet: View {
    @ObservedObject private var lib = ExeLibrary.shared
    @Environment(\.dismiss) private var dismiss
    ...
    var body: some View {
        NavigationStack {
            Group {
                if lib.scanning {
                    ProgressView("Scanning drive_c for games...")
                } else if lib.games.isEmpty {
                    VStack(spacing: 12) {
                        Image(systemName: "externaldrive.badge.questionmark")
                            .font(.system(size: 42))
                            .foregroundColor(.secondary)
                        Text("No .exe files found")
                            .font(.headline)
                        Text("Copy your game folder into the app's Documents/wine/drive_c/ (e.g. via the Files app or SideStore file sharing), then tap Rescan.\n\nGames land at C:\\\\... from Wine's perspective.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                            .multilineTextAlignment(.center)
                        Button("Rescan") { lib.scan() }
                            .buttonStyle(.borderedProminent)
                    }
                    .padding()
                } else {
                    List(lib.games) { g in
                        Button {
                            lib.launch(g, args: customArgs, desktop: useDesktop)
                            dismiss()
                        } label: {
                            VStack(alignment: .leading, spacing: 2) {
                                Text(g.displayName)
                                    ...
                                    .foregroundColor(.primary)
                                Text("\(g.folder) — \(String(format: "%.0f", g.sizeMB)) MB\n\(g.winePath)")
                                    .font(.caption2)
                                    .foregroundColor(.secondary)
                                    .lineLimit(2)
                            }
            ...
            .safeAreaInset(edge: .bottom) {
                VStack(alignment: .leading, spacing: 8) {
                    Toggle("Virtual desktop 1280×720", isOn: $useDesktop)
                        .font(.caption)
                    HStack {
                        Text("Args")
                            .font(.caption)
                            .foregroundColor(.secondary)
                        TextField("e.g. -dx11 -windowed", text: $customArgs)
